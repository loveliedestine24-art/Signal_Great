...
# CSS pou Style nwa ak bèl bwat siyal
st.markdown("""<style>.stApp { background-color: #050505; color: white; } .signal-call { padding: 20px; border-radius: 10px; background-color: #0b2e13; border: 2px solid #28a745; text-align: center; font-weight: bold; font-size: 20px; } .signal-put { padding: 20px; border-radius: 10px; background-color: #2c0b0e; border: 2px solid #dc3545; text-align: center; font-weight: bold; font-size: 20px; }</style>""", unsafe_allow_html=True)
...
if st.button("EKZEKITE ANALIZ"):
    with st.spinner("Bot la ap analize bouji yo..."):
        ...
        if not data.empty:
            ...
            if last['Close'] <= last['Lower'] and last['RSI'] < 35 and last['Close'] > last['EMA200']:
                ...
                st.warning("Pa gen siyal solid kounye a. Tann mache a touche limit yo.")
